...
function getRound(radix, key, tweak, x) {
    ...
    const maxLen = 2 ** 32 - 1;
    // 2 â‰¤ minlen â‰¤ maxlen < 2**32
    if (2 > minLen || minLen > maxLen || maxLen >= 2 ** 32)
        throw new Error('Invalid radix: 2 â‰¤ minlen â‰¤ maxlen < 2**32');
    if (!Array.isArray(x))
    ...
    // Q = T || [0](âˆ’tâˆ’bâˆ’1) mod 16 || [i]1 || [NUMradix(B)]b.
    ...
    const round = (A, B, i, decrypt = false) => {
        ...
        // Let S be the first d bytes of the following string of âŽ¡ï£®d/16âŽ¤ï£¹ blocks:
        // R || CIPHK(R âŠ•[1]16) || CIPHK(R âŠ•[2]16) ...CIPHK(R âŠ•[âŽ¡ï£®d / 16âŽ¤ï£¹ â€“ 1]16).
        let s = Array.from(r);
