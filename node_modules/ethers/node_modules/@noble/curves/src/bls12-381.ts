...
// G2 is the order-q subgroup of E2(FpÂ²) : yÂ² = xÂ³+4(1+âˆšâˆ’1),
// where Fp2 is Fp[âˆšâˆ’1]/(x2+1). #E2(Fp2 ) = h2q, where
// GÂ² - 1
// h2q
...
const Fp2: mod.IField<Fp2> & Fp2Utils = {
  ...
  inv: ({ c0: a, c1: b }) => {
    ...
    // (a + bu)(a - bu) = aÂ² + bÂ²
    //
    // which holds because uÂ² = -1. This can be rewritten as
    //
    // (a + bu)(a - bu)/(aÂ² + bÂ²) = 1
    //
    // because aÂ² + bÂ² = 0 has no nonzero solutions for (a, b).
    // This gives that (a - bu)/(aÂ² + bÂ²) is the inverse
...
const Fp6Square = ({ c0, c1, c2 }: Fp6) => {
  let t0 = Fp2.sqr(c0); // c0Â²
  let t1 = Fp2.mul(Fp2.mul(c0, c1), _2n); // 2 * c0 * c1
  let t3 = Fp2.mul(Fp2.mul(c1, c2), _2n); // 2 * c1 * c2
  let t4 = Fp2.sqr(c2); // c2Â²
  return {
...
const Fp6: mod.IField<Fp6> & Fp6Utils = {
  ...
  inv: ({ c0, c1, c2 }) => {
    let t0 = Fp2.sub(Fp2.sqr(c0), Fp2.mulByNonresidue(Fp2.mul(c2, c1))); // c0Â² - c2 * c1 * (u + 1)
    let t1 = Fp2.sub(Fp2.mulByNonresidue(Fp2.sqr(c2)), Fp2.mul(c0, c1)); // c2Â² * (u + 1) - c0 * c1
    let t2 = Fp2.sub(Fp2.sqr(c1), Fp2.mul(c0, c2)); // c1Â² - c0 * c2
...
function Fp4Square(a: Fp2, b: Fp2): { first: Fp2; second: Fp2 } {
  ...
  const b2 = Fp2.sqr(b);
  return {
    first: Fp2.add(Fp2.mulByNonresidue(b2), a2), // bÂ² * Nonresidue + aÂ²
    second: Fp2.sub(Fp2.sub(Fp2.sqr(Fp2.add(a, b)), a2), b2), // (a + b)Â² - aÂ² - bÂ²
  };
}
type Fp12Utils = {
...
const Fp12: mod.IField<Fp12> & Fp12Utils = {
  ...
  inv: ({ c0, c1 }) => {
    let t = Fp6.inv(Fp6.sub(Fp6.sqr(c0), Fp6.mulByNonresidue(Fp6.sqr(c1)))); // 1 / (c0Â² - c1Â² * v)
  ...
  finalExponentiate: (num) => {
    ...
    // (t2 * t5)^(qÂ²) * (t4 * t1)^(qÂ³) * (t6 * t1.conj)^(q^1) * t7 * t3.conj * t1
...
// SWU Map - Fp2 to G2': yÂ² = xÂ³ + 240i * x + 1012 + 1012i
...
export const bls12_381: CurveFn<Fp, Fp2, Fp6, Fp12> = bls({
  ...
  // G1 is the order-q subgroup of E1(Fp) : yÂ² = xÂ³ + 4, #E1(Fp) = h1q, where
  // characteristic; z + (zâ´ - zÂ² + 1)(z - 1)Â²/3
  G1: {
    Fp,
    // cofactor; (z - 1)Â²/3
    ...
    isTorsionFree: (c, point): boolean => {
      ...
      // const Q = S.double();
      // const S2 = S.sigma();
      // // [(zÂ² âˆ’ 1)/3](2Ïƒ(P) âˆ’ P âˆ’ ÏƒÂ²(P)) âˆ’ ÏƒÂ²(P) = O
    ...
    fromBytes: (bytes: Uint8Array): AffinePoint<Fp> => {
      ...
      if (bytes.length === 48) {
        ...
        const right = Fp.add(Fp.pow(x, _3n), Fp.create(bls12_381.params.G1b)); // yÂ² = xÂ³ + b
        let y = Fp.sqrt(right);
  ...
  G2: {
    ...
    isTorsionFree: (c, P): boolean => {
      return P.multiplyUnsafe(bls12_381.params.x).negate().equals(G2psi(c, P)); // Ïˆ(P) == [u](P)
      // Older version: https://eprint.iacr.org/2019/814.pdf
      // Î¨Â²(P) => Î¨Â³(P) => [z]Î¨Â³(P) where z = -x => [z]Î¨Â³(P) - Î¨Â²(P) + P == O
    ...
    clearCofactor: (c, P) => {
      ...
      let t2 = G2psi(c, P);                   // Î¨(P)
      let t3 = P.double();                    // 2P
      t3 = G2psi2(c, t3);                     // Î¨Â²(2P)
      t3 = t3.subtract(t2);                   // Î¨Â²(2P) - Î¨(P)
      t2 = t1.add(t2);                        // [-x]P + Î¨(P)
      t2 = t2.multiplyUnsafe(x).negate();     // [xÂ²]P - [x]Î¨(P)
      t3 = t3.add(t2);                        // Î¨Â²(2P) - Î¨(P) + [xÂ²]P - [x]Î¨(P)
      t3 = t3.subtract(t1);                   // Î¨Â²(2P) - Î¨(P) + [xÂ²]P - [x]Î¨(P) + [x]P
      const Q = t3.subtract(P);               // Î¨Â²(2P) - Î¨(P) + [xÂ²]P - [x]Î¨(P) + [x]P - 1P
      return Q;                               // [xÂ²-x-1]P + [x-1]Î¨(P) + Î¨Â²(2P)
    },
    fromBytes: (bytes: Uint8Array): AffinePoint<Fp2> => {
      ...
      if (bytes.length === 96 && bitC) {
        ...
        const right = Fp2.add(Fp2.pow(x, _3n), b); // yÂ² = xÂ³ + 4 * (u+1) = xÂ³ + b
    ...
    Signature: {
      ...
      fromHex(hex: Hex): ProjPointType<Fp2> {
        ...
        const y2 = Fp2.add(Fp2.pow(x, _3n), bls12_381.params.G2b); // yÂ² = xÂ³ + 4
        // The slow part
        let y = Fp2.sqrt(y2);
  ...
  params: {
    ...
    r: Fr.ORDER, // order; zâ´ âˆ’ zÂ² + 1; CURVE.n from other curves
  },
  htfDefaults,
  hash: sha256,
  randomBytes,
